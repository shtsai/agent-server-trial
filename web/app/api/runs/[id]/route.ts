// The agent is internal on both platforms, so this route is the only thing that knows its address.
// It forwards the status code UNCHANGED: a 404 here carries the reason the run is gone, and
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${AGENT()}/runs/${id}?after=${after}`, {
